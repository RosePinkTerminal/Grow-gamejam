...
func DrawStore(showStore bool, textures map[string]rl.Texture2D, player *Player, buyOrSell bool) {
	...
	if showStore {
		...
		rl.DrawTexture(textures["shopkeeper"], 300, 120, rl.White)

		rl.DrawText("Buy", 120, 350, 40, rl.DarkGreen)
		rl.DrawText("Sell", 220, 350, 40, rl.Red)
		...
		if buyOrSell {
			for _, item := range items {
				...
				if buyOrSell && item.Buying {
					rl.DrawTextureEx(item.Image, rl.NewVector2(120, 150+1*80), 0, 2.0, rl.White)

					rl.DrawText(fmt.Sprintf("$%d", item.Price), 120, int32(185+1*120), 20, rl.Black)
					rl.DrawText("Currently buying!", 120, 150, 20, rl.DarkGreen)

					if rl.IsMouseButtonPressed(rl.MouseButtonLeft) {
						mouseX := rl.GetMouseX()
						mouseY := rl.GetMouseY()
						if mouseX >= 120 && mouseX <= 170 && mouseY >= 220 && mouseY <= 285 {

							if player.inventory.AddItem("carrot_seed", 5, 1) == nil && player.money >= 5 {
			...
			for _, item := range items {
				...
				if !item.Buying {
					...
					if rl.IsMouseButtonPressed(rl.MouseButtonLeft) {
						mouseX := rl.GetMouseX()
						mouseY := rl.GetMouseY()
						if mouseX >= 120 && mouseX <= 170 && mouseY >= 230 && mouseY <= 285 {
